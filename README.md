...
版本：`3.3.0` · 作者：Red · Windows 10/11 · Python 3.10+
...
- **女明星角色卡**：姓名、出生日期、身高、国籍/公司、性格关键词、语言风格、爱好、代表作品、当前日程等字段，CRUD 管理
- **男主角设定**：在设置面板中填写姓名、籍贯、外貌等字段
- **小说提示词模板**：在设置面板中粘贴完整 system prompt，三者自动拼接后发送
- AI 回复自动解析编号选项（1-4），渲染为可点击的章回选项按钮
- 选项按钮点击后自动填入输入框，一键继续故事
...
| 版本 | 日期 | 内容 |
|------|------|------|
| 3.3.0 | 2026-05-22 | 故事模式角色卡 CRUD、小说模式女明星角色卡 CRUD + 男主角设定 + 模板、overlay 闪烁修复 |
| 3.2.1 | 2026-05-21 | Edge TTS 神经网络朗读（28 个中文声线）、生成动画换 mona 猫猫 |
