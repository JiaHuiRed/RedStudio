...
[![版本](https://img.shields.io/badge/版本-v3.4.1-blue)](CHANGELOG.md)
...
- **规则强化**：每轮自动注入当前好感度、阶段、行为边界提醒；多阶段行为规则表开局告知 AI；选项缺失时内联警告 + 重新生成按钮
- **作者注记**：可折叠，插入 context 末尾靠后位置，静默修正 AI 行为
- **记忆压缩**：对话轮次过多时，一键将旧消息摘要压缩，释放上下文空间
- **快速预设**：弹出面板，一键切换模式与场景（言情 / 奇幻RPG / 校园 / 武侠）
- **新故事设定**：主角名称、故事方向、叙事视角（一/二/三人称）、字数、好感起始值
- **自定义阶段行为**：每个好感度阶段可独立编写行为规则，AI 严格遵循当前阶段边界
- **自动推进**：点击选项自动发送，好感度变化实时浮动反馈
...
- **历史搜索**：侧边栏实时过滤历史对话
- **精简侧边栏**：冗余标题移除，RPG/小说共用动态区域，快速预设改为弹出面板
