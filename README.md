...
[![版本](https://img.shields.io/badge/版本-v3.7.1-blue)](CHANGELOG.md)
...
面向 DeepSeek / Qwen / Claude / Ollama 等大模型 API 的 Windows 桌面对话客户端。支持 **P5R 风格 JRPG 角色扮演**（元素克制 + 技能系统 + 好感养成）、**小说互动写作**（模板驱动 + 选项按钮）、流式输出、Markdown 渲染、TTS 朗读等功能。
...
- ✏️ **作者注记**：可折叠，自动注入 context 末尾附近，临时修正 AI 行为
- ⏹ **停止生成**：流式输出过程中可随时中断
- 📋 **消息复制 / 朗读**：每条 AI 回复提供复制全文和 TTS 朗读按钮

## 🎮 JRPG 模式（P5R 风格角色扮演）

- **P5R 战斗属性**：力/魔/速/防/体力，HP 基于体力、MP 基于魔力
- **元素系统**：9 种元素（🔥💧🌍🌪❄️⚡🌳✨🌑），克制 2 倍 / 被克制 0.5 倍
- **130 个技能库**：10 元素 × 13 技能（5 初级/4 中级/3 高级/1 究极），分物理/魔法/辅助三类，战斗中自动注入
- **技能管理**：最多装备 4 个技能，可切换/遗忘/找回，AI 管理技能状态
- **5 位女 NPC**：每次新游戏全随机生成（名字/性格/外貌/元素各不相同），支持保存到角色库复用
- **好感度系统**：0–100 进度条 + 4 阶段（陌生人→朋友→暧昧→恋人），AI 通过 `[FAV:角色名±N]` 更新
- **NPC 面板**：P5R 协力者卡片风格，显示元素角标、性格标签、Rank 等级、好感进度条
- **4 维社交属性（德智体美）**：影响对话选项解锁，魅力直接影响好感度获取倍率
- **升级养成**：HP/MP/Lv/EXP/GOLD 实时同步，升级弹出通知
- **战斗提示**：战斗选项自动显示 🟢 效果拔群 / 🔴 效果不好
- **一键导入角色卡**：AI 输出角色卡时渲染导入按钮，保存到角色库
- **角色卡分类**：NPC 首次登场强制输出角色卡，后续不再重复
- **历史分类**：侧边栏按 JRPG/小说/全部过滤历史对话

## 📖 小说模式（互动写作）

- **模板系统**：设置面板管理自定义 prompt 模板，内置"言情宿舍""体育竞技"预置，支持占位符（{hero_name}/{story_dir}/{word_count}/{pov}）
- **女主角角色卡**：姓名、身材、外貌、性格、语言风格、爱好、出身
- **选项格式**：`[CHOICES]` 块 / `A. B. C. D.` / `1. 2. 3. 4.` 自动渲染为可点击按钮
- **模板驱动**：故事方向、好感度规则等完全由用户提示词定义，系统不自动注入
- **作者注记**：可折叠，插入 context 靠后位置，静默修正 AI 行为
- **记忆压缩**：对话轮次过多时，一键将旧消息摘要压缩，释放上下文空间
- **历史重命名**：对话历史条目支持改名（自定义弹窗）
...
- **小米 MiMo TTS**：限时免费，4 个中文声线，长文本自动分段并行下载
...
- **macOS 风格**：无边框窗口、红黄绿交通灯按钮、36px 紧凑标题栏
- **毛玻璃效果**：标题栏和侧边栏半透明模糊（backdrop-filter）
- **5 套主题**：亮色 / 护眼米 / 护眼绿 / 深蓝 / 纯黑（OLED），循环切换
- **窗口自由缩放**：鼠标拖拽任意边/角缩放，无 backdrop-filter 闪烁
- **历史搜索 & 分类**：侧边栏实时过滤历史对话，按 JRPG/小说分类
- **应用图标**：多分辨率 ICO（16/32/48/64/128/256）
